...
export const JournalPublication=()=>{
    ...
    const generatePDF = async (report_id)=> {
        ...
        doc.text('Name of Journal', 14, 88);
        doc.setFont("times", "");
  ...
  console.log(filter)
   ...
   const onClickFilter=async()=>{
    ...
    try{
        ...
        const filteredRecords=await axios.post(`http://localhost:1234/setaf/filterSetaf/data_setaf_journal_publication/${logged.faculty_id}`,filter)
...
try{
    ...
    const fetchNptelRecords=async()=>{
        try{
            const temp = await NptelRecords(`${logged.faculty_id}`)
            // console.log(temp.data)
...
/>
       ...
       <div class="report-header">
       <h1 class="recent-Articles">Your Reports</h1>
       <a className="topic-headings" href="/Setaf/SetafForms/visitindustry"><button style={{top:"27.5px",right:"50px"}} class="views" >+ Add</button></a>
       </div>
       <table className='table table-striped'>
        <thead>
        <tr className="table-active">
...
export const Seed=()=>{
  ...
  const generatePDF = async (report_id)=> {
      ...
      doc.text('SETAF', 97, 45);
      doc.rect(58,54,94,11)
      doc.text("Seed money Proposal for Research",67,61)
      
      doc.rect(168, 54, 30, 11).stroke();
      doc.setFontSize(15); 
      doc.text(`${data.acd_yr}`,172,61)
      doc.rect(12, 80, 60, 12).stroke();
      doc.text('Title of the Project', 14, 88);
      doc.setFont("times", "");
      doc.rect(72, 80, 127, 12).stroke();
      doc.text(`${data.title_of_the_research_project}`, 77, 88);
      
      doc.setFont("times", "bold");
      doc.rect(12, 92, 60, 12).stroke();
      doc.text('Name of the Faculty', 14, 100);
      doc.setFont("times", "");
      doc.rect(72, 92, 127, 12).stroke();
      doc.text(`${data.name_of_the_faculty}`, 77, 100);
      
      doc.setFont("times", "bold");
      doc.rect(12, 104, 60, 12).stroke();
      doc.text('Amout of Seed money', 14, 112);
      doc.setFont("times", "");
      doc.rect(72, 104, 127, 12).stroke();
      doc.text(`${data.amount_of_seed_money}`, 77, 112);
...
/>
       ...
       <div class="report-header">
       <h1 class="recent-Articles">Your Reports</h1>
       <a className="topic-headings" href="/Setaf/SetafForms/seed"><button style={{top:"27.5px",right:"50px"}} class="views" id="addButton">+ Add</button></a>
       </div>
       <table className='table table-striped'>
        <thead>
        <tr className="table-active">
   ...
   const generatePDF = async (report_id)=> {
      ...
      try{
       ...
       pdfDocument = await getDocument({ data: pdfData }).promise;
         ...
         doc.text('SETAF', 97, 45);
         doc.rect(46,54,120,11)
         doc.text("COLLABRATIVE ACTIVITIES WITH MOU SIGNED INDUSTRIES, INSTITUTION",59,61)
...
try{
  ...
  const fetchAwardRecords=async()=>{
      try{
          const temp = await AwardAtNationalList(`${logged.faculty_id}`)
      // console.log(temp.data)
      setAward(temp.data)
      }
      catch(e){
...
try{
  ...
  const fetchBooksRecords=async()=>{
      try{
          const temp = await BooksList(`${logged.faculty_id}`)
      // console.log(temp.data)
      setBooks(temp.data)
      }
      catch(e){
...
//////////////////STUDENT MOTIVATION//////////////////
export const StudentsMotivation=()=>{
...
/>
       ...
       <th>Paper Presentation/Project Submission/Other Contest</th>
       <th>Date</th>
       <th>No of beneficiaries</th>
       <th>Download PDF</th>
