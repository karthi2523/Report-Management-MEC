  ...
  const depts = dept.map((val) => ({
     ...
     const onClickFilter=async()=>{
      ...
      try{
          ...
          const filteredRecords=await axios.post(`http://localhost:1234/setaf/filterSetaf/principal/data_setaf_journal_publication`,filter)
...
///////////////workshop/////////////
export const WorkshopPrincipalDashboard=()=>{
...
//fetch   
  ...
  const fetchWorkshopRecords=async()=>{
     try{
      const temp = await workshopPrincipalView(`${logged.dept_id}`)
      // console.log(temp.data)
...
export const TechtalPrincipalView=()=>{
  ...
  const generatePDF = async (report_id)=> {
      ...
      let pdfDocument2
      try{
          const pdfUrl = `/Journal_SETAF/${data.attendance_sheet_pdf,data.handout_of_lecture_pdf}`;
          const pdfResponse = await axios.get(pdfUrl, { responseType: 'arraybuffer' });
          const pdfData = pdfResponse.data;
...
export const FacultyGuestTalkPrincipalDashboard=()=>{
  const generatePDF = async (report_id)=> {
      try{
          // alert(report_id)
...
//////////////////////facultyView page fetch code/////////////////////
  ...
  const fetchFacultyRecords=async()=>{
      try{
          const temp = await facultyPrincipalView()
      // console.log(temp.data)
...
export const NptelprincipalDashboard=()=>{
 ...
 ///////////////NPTEL fetch code///////////////
  ...
  const fetchNptelRecords=async()=>{
      try{
          const temp = await nptelPrincipalView()
          // console.log(temp.data)
 ...
 export const TasteprincipalDashboard=()=>{
      ...
      //////TASTE fetch Code////
          ...
          const fetchTasteRecords=async()=>{
              try{
              const temp = await TastePrincipalView()
              // console.log(temp.data)
...
//////////////////////////////////////////////seed money proposal///////////////////////////////////////////
export const SeedPrincipalDashboard=()=>{
...
///seed fetch code////
  ...
  const fetchSeedRecords=async()=>{
      try{
      const temp = await seedPrincipalView()
      // console.log(temp.data)
...
export const ProposalPrincipaldashboard=()=>{
  ...
  const generatePDF = async (report_id)=> {
    ...
    doc.setFont("times", "");
    doc.rect(92, 116, 110, 12).stroke();
    doc.text(`${data.amount_quoted_in_lakhs}`, 98, 124);
...
//fetch
  ...
  const fetchIndustryRecords=async()=>{
      ...
      try{
      const temp = await industryPrincipalView()
      // console.log(temp.data)
...
////////////data fetch code/////////////////
  ...
  const fetchAwardRecords=async()=>{
      try{
          const temp = await AwardAtNationalPrincipal()
      // console.log(temp.data)
      setAward(temp.data)
      }
      catch(e){
...
catch(e)
{
 ...
 const depts = dept.map((val) => ({
  ...
  const fetchBooksRecords=async()=>{
      try{
          const temp = await BooksPrincipal()
      // console.log(temp.data)
      setBooks(temp.data)
      }
      catch(e){
